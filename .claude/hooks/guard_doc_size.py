...
def is_rule_doc(path: Path) -> bool:
...
def main() -> int:
    ...
    if not is_rule_doc(target):
        return 0

    content = prospective_content(payload)
    line_count = len(content.splitlines()) or 1
    if line_count > MAX_RULE_DOC_LINES:
        deny(
            f"AGENTS.md and CLAUDE.md may not exceed {MAX_RULE_DOC_LINES} lines. "
